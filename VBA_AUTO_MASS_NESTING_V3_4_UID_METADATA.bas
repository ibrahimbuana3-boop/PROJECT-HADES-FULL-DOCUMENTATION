Attribute VB_Name = "HADES_AMN_V34_UID"
Option Explicit

'==========================================================
' PROJECT HADES
' HADES AUTO MASS NESTING V3.4 UID - VISUAL LIGATURE BREAKER + AREA GRID WRAP
'
' SHORTCUT UTAMA:
...
' HADES_AUTO_MASS_NESTING_V3_4_LIGATURE_BREAKER
' HADES_AUTO_MASS_NESTING_V3_4_UID_METADATA
'
' BASIS:
...
' - AREA WRAP: setelah area 178 ke-6, area berikutnya lanjut dari kiri bawah.
' - VISUAL LIGATURE BREAKER: memecah pasangan F+F / F+I / F+L dengan ZWNJ.
'
' FITUR V3:
...
' - Support Unicode Ligature Cleaner + Visual Ligature Breaker seperti QC Auto Rename V5.3.
...
' - Support Pattern Catalog V3.3 UID Marker pada Shape.Name.
' - Memberi metadata HADES_AMN|ROW=... pada hasil duplicate untuk QC TYPO NEST.
' - Membersihkan marker source HADES_PC_UID setelah nesting sukses, tetapi menjaga HADES_AMN.
'==========================================================
...
Private Const HAMN_AREA_SPACING As Double = 6#
Private Const HAMN_AREA_MAX_PER_ROW As Long = 6
Private Const HAMN_AREA_ROW_SPACING As Double = 6#
Private Const HAMN_OUTPUT_OFFSET_RIGHT As Double = 10# 'Legacy; tidak dipakai di V3.3 page anchor.
Private Const HAMN_PAGE_TOP_OFFSET As Double = 640#
...
Private Const HAMN_OUTPUT_META_PREFIX As String = "HADES_AMN|"

Private Const HAMN_MIN_TEXT_H As Double = 1#
Private Const HAMN_ENABLE_VISUAL_LIGATURE_BREAKER As Boolean = True
...
Public Sub HADES_AUTO_MASS_NESTING_V33()
    HADES_AUTO_MASS_NESTING_V3_4_LIGATURE_BREAKER
End Sub

Public Sub HADES_AUTO_MASS_NESTING_V34()
    HADES_AUTO_MASS_NESTING_V3_4_LIGATURE_BREAKER
End Sub

Public Sub HADES_AUTO_MASS_NESTING_V3_4_UID_METADATA()
    HADES_AUTO_MASS_NESTING_V3_4_LIGATURE_BREAKER
End Sub


Public Sub HADES_AUTO_MASS_NESTING_V3_4_LIGATURE_BREAKER()
    ...
    If ActiveDocument Is Nothing Then
        MsgBox "Tidak ada dokumen aktif.", vbExclamation, "Hades Auto Mass Nesting V3.4 UID"
        Exit Sub
    End If
    ...
    If ActiveSelection.Shapes.Count = 0 Then
        MsgBox "Select semua panel master yang sudah loose dulu." & vbCrLf & vbCrLf & _
               ...
               vbExclamation, "Hades Auto Mass Nesting V3.3"
        Exit Sub
    End If
    ...
    If Dir$(catalogPath) = "" Then
        MsgBox "Pattern Catalog belum ditemukan." & vbCrLf & vbCrLf & _
               ...
               vbCritical, "Hades Auto Mass Nesting V3.3"
    ...
    If Not HAMN_LoadCatalog(catalogPath, catN, catSize, catPanelNo, catW, catH, catType, catName, catBucket, catUID, catOrigName) Then
        MsgBox "Gagal membaca Pattern Catalog.", vbCritical, "Hades Auto Mass Nesting V3.3"
        ActiveDocument.Unit = oldUnit
        Exit Sub
    End If

    If catN = 0 Then
        MsgBox "Pattern Catalog kosong / tidak ada panel.", vbCritical, "Hades Auto Mass Nesting V3.3"
        ActiveDocument.Unit = oldUnit
        Exit Sub
    End If
    ...
    If Not HAMN_LoadOrderRecords(orderPath, orderN, orderSize, orderName, orderNo, orderNick, orderQty, orderSeqMap) Then
        MsgBox "Gagal membaca Order.txt.", vbCritical, "Hades Auto Mass Nesting V3.3"
        ActiveDocument.Unit = oldUnit
        Exit Sub
    End If

    If orderN = 0 Or orderQty.Count = 0 Then
        MsgBox "Order.txt tidak berisi record order valid.", vbCritical, "Hades Auto Mass Nesting V3.3"
        ActiveDocument.Unit = oldUnit
        Exit Sub
    End If
    ...
    For i = 1 To srcN
        ...
        srcUID(i) = HAMN_ExtractUIDFromMarkerName(srcShapes(i).Name)
        srcUsed(i) = False
    Next i
    ...
    ActiveDocument.BeginCommandGroup "HADES AUTO MASS NESTING V3.3 AREA GRID WRAP"
    cmdStarted = True
    ...
    For i = 1 To catN
        ...
        For q = 1 To qty
            ...
            HAMN_SetDuplicateMetadata dup, recIdx, catSize(i), catBucket(i), catPanelNo(i), catUID(i), q
...
NextCatalogItem:
    ...
    If outN = 0 Then
        ...
        MsgBox "Tidak ada panel yang berhasil dibuat." & vbCrLf & vbCrLf & _
               warn, vbCritical, "Hades Auto Mass Nesting V3.3"
        Exit Sub
    End If
    ...
    MsgBox "HADES AUTO MASS NESTING V3.4 UID selesai." & vbCrLf & vbCrLf & _
           ...
           vbInformation, "Hades Auto Mass Nesting V3.3"
...
ErrHandler:
    ...
    MsgBox "ERROR HADES_AUTO_MASS_NESTING_V3:" & vbCrLf & _
           err.Description, vbCritical, "Hades Auto Mass Nesting V3.3"
        ...
        If ln <> "" Then
            ...
            If UCase$(Left$(ln, 6)) = "PANEL|" Then
                ...
                If UBound(p) >= 4 Then
                    ...
                    'Pattern Catalog V3.3+:
                    'PANEL|SIZE|PANEL_NO|W|H|TYPE|NAME|BUCKET|LEFT|TOP|CX|CY|LOCAL_CX|LOCAL_CY|SIZEDB|PANEL_UID|ORIGINAL_NAME
                    catUID(catN) = ""
                    catOrigName(catN) = catName(catN)

                    If UBound(p) >= 15 Then catUID(catN) = Trim$(CStr(p(15)))
...
Private Function HAMN_FindMatchingSourceSmart( _
    ...
    'Prioritas utama: cocokkan source panel dengan UID dari Pattern Catalog V3.3.
    If targetUID <> "" Then
        For i = 1 To srcN
    ...
    'Fallback kompatibel: jika catalog lama / marker hilang, tetap pakai dimensi.
...
Private Function HAMN_SetTextSafe(ByVal t As Shape, ByVal newText As String) As Boolean
    ...
    newText = HAMN_BreakVisualLigatures(newText)
...
'==========================================================
' VISUAL LIGATURE BREAKER � CHIP FROM QC AUTO RENAME V5.3
'==========================================================
' Unicode ligature cleaner hanya memperbaiki karakter ligature
' seperti ? / ?. Tetapi beberapa font jersey tetap bisa
' menggambar huruf biasa seperti FI / FL / FF menjadi bentuk menyatu
' karena fitur OpenType ligature.
'
' Solusi: sisipkan Zero Width Non-Joiner (U+200C) di antara pasangan
' F+F, F+I, F+L agar secara visual tidak menyatu, tetapi teks tetap
' terlihat normal untuk operator.

Private Function HAMN_BreakVisualLigatures(ByVal s As String) As String

    If Not HAMN_ENABLE_VISUAL_LIGATURE_BREAKER Then
        HAMN_BreakVisualLigatures = s
        Exit Function
    End If
    ...
    For i = 1 To Len(s)
        ...
        If i < Len(s) Then
            ...
            If HAMN_IsVisualLigaturePair(ch, nx) Then
                r = r & z
            End If
        End If
...
Private Function HAMN_IsVisualLigaturePair(ByVal a As String, ByVal b As String) As Boolean
    ...
    If a = "F" Then
        Select Case b
            Case "F", "I", "L"
                HAMN_IsVisualLigaturePair = True
        End Select
    End If
...
Private Function HAMN_LayoutShelf( _
    ...
    ' V3.3 AREA GRID WRAP
    '======================================================
    ' Versi lama menambah area 178 secara horizontal terus:
    ' 178(1), 178(2), 178(3), 178(4), 178(5), 178(6), 178(7), dst.
    '
    ' Versi ini membatasi jumlah area per baris.
    ' Setelah 178(6), 178(7) mulai lagi dari kiri bawah.
    '
    ' Contoh:
    ...
    logText = "AREA_GRID_WRAP|MAX_PER_ROW=" & CStr(HAMN_AREA_MAX_PER_ROW) & _
              "|AREA_ROW_SPACING=" & HAMN_DblToStr(HAMN_AREA_ROW_SPACING) & vbCrLf
    ...
    For i = 1 To outN
        ...
        If w > HAMN_AREA_W Or h > HAMN_AREA_H Then
            ...
            GoTo NextOutputShape
        End If

        ' Penuhi horizontal dulu dalam satu area 178.
        If curX > 0# Then
            If curX + w > HAMN_AREA_W Then
                curX = 0#
                curY = curY + rowH + HAMN_GAP
                rowH = 0#
            End If
        End If

        ' Jika tinggi area penuh, pindah ke area 178 berikutnya.
        If curY + h > HAMN_AREA_H Then
        ...
        ' AREA WRAP:
        ' area 1-6 = baris area pertama
        ' area 7-12 = baris area kedua, mulai dari kiri bawah
        areaCol = (areaIndex - 1) Mod HAMN_AREA_MAX_PER_ROW
        areaRowBlock = Int((areaIndex - 1) / HAMN_AREA_MAX_PER_ROW)

        areaLeft = baseLeft + (CDbl(areaCol) * (HAMN_AREA_W + HAMN_AREA_SPACING))
        areaTop = baseTop - (CDbl(areaRowBlock) * (HAMN_AREA_H + HAMN_AREA_ROW_SPACING))

        targetLeft = areaLeft + curX
        targetTop = areaTop - curY

        HAMN_MoveShapeTopLeft outShapes(i), targetLeft, targetTop

        logText = logText & _
                  "AREA " & CStr(areaIndex) & _
                  " | ROWBLOCK=" & CStr(areaRowBlock + 1) & _
                  " | COL=" & CStr(areaCol + 1) & _
                  " | " & outBucket(i) & " | " & _
                  outLabel(i) & " | " & _
                  HAMN_DblToStr(w) & " x " & HAMN_DblToStr(h) & _
                  " | X=" & HAMN_DblToStr(curX) & _
                  " Y=" & HAMN_DblToStr(curY) & vbCrLf

        curX = curX + w + HAMN_GAP
...
Private Function HAMN_ExtractUIDFromMarkerName(ByVal markerName As String) As String

    Dim p() As String

    markerName = Trim$(markerName)

    If UCase$(Left$(markerName, Len(HAMN_MARK_PREFIX))) = UCase$(HAMN_MARK_PREFIX) Then
        p = Split(markerName, "|")
        If UBound(p) >= 1 Then
            HAMN_ExtractUIDFromMarkerName = Trim$(CStr(p(1)))
            Exit Function
        End If
    End If

    HAMN_ExtractUIDFromMarkerName = ""

End Function


Private Sub HAMN_SetDuplicateMetadata( _
    ByVal shp As Shape, _
    ByVal orderRow As Long, _
    ByVal sz As String, _
    ByVal panelBucket As String, _
    ByVal panelNo As Long, _
    ByVal panelUID As String, _
    ByVal copySeq As Long _
)

    On Error Resume Next

    If panelUID = "" Then panelUID = HAMN_BuildLegacyUID(sz, panelBucket, panelNo)

    shp.Name = HAMN_OUTPUT_META_PREFIX & _
               "ROW=" & CStr(orderRow) & _
               "|SIZE=" & HAMN_NormalizeSize(sz) & _
               "|PANEL=" & HAMN_NormalizeBucket(panelBucket) & _
               "|PANELNO=" & CStr(panelNo) & _
               "|UID=" & panelUID & _
               "|COPY=" & CStr(copySeq)
...
Private Sub HAMN_CleanPatternMarkersFromDocument(ByRef logText As String)
    ...
    logText = logText & "MARKER_CLEANUP|HADES_PC_UID_CLEANED=" & CStr(cleaned) & vbCrLf & _
                        "MARKER_CLEANUP|HADES_AMN_METADATA_KEPT=YES" & vbCrLf
...
Private Sub HAMN_WriteReport( _
    ...
    Print #f, "PROJECT HADES - AUTO MASS NESTING REPORT V3 AUTO RENAME"
    ...
    Print #f, "METHOD=BOUNDING_BOX_SAFE_SHELF"
    ...
    Print #f, "TOTAL_AREA_USED=" & CStr(areaCount)
    Print #f, ""
